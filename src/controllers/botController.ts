...
function startGameWithBot(room: Room, humanPlayer: Player, botPlayer: Player) {
  ...
  game.currentTurn =
    Math.random() < 0.5 ? humanPlayer.clientId : botPlayer.clientId;
}
...
function generateCoordinatesForAttack(data: PlayerGameData): {
  x: number;
  y: number;
} {
...
function processAttack(
  ...
  x: number,
  y: number
): { status: 'miss' | 'shot' | 'killed'; gameOver: boolean } {
...
function sendFinishMessage(game: Game, winnerId: string) {
  const humanPlayerId = Object.keys(game.players).find(
